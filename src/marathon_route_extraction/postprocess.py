...
def postprocess_mask(
    ...
    return (skeletonize(mask > 127) * 255).astype(np.uint8)
...
def _filter_noise(mask, area_thresh, circ_thresh, skel_thresh):
    ...
    for lbl in range(1, num):
        ...
        if area < area_thresh and _circularity(area, comp) > circ_thresh and int(skeletonize(comp > 0).sum()) < skel_thresh:
            out[labels == lbl] = 0
